  ...
  for (let i = 0; i < dados.length; i++) {
    if (dados[i].checked) {
      if (dados[i].value == Spain.hospedagem.hospedagem1.nomeHospedagem1) {
        ...
        dados[i].value == Spain.hospedagem.hospedagem2.nomeHospedagem2
      ) {
        valorSoma = Spain.hospedagem.hospedagem2.valor;
      } else if (
        dados[i].value == Spain.hospedagem.hospedagem3.nomeHospedagem3
      ) {
        valorSoma = Spain.hospedagem.hospedagem3.valor;
      } else if (
        dados[i].value == Spain.hospedagem.hospedagem4.nomeHospedagem4
      ) {
...
function somaTurismo() {
  ...
  for (let i = 0; i < dados.length; i++) {
    if (dados[i].checked) {
      if (
        dados[i].value == Spain.pontosTuristicos.primeiroPontoTuristico.nome
      ) {
        valorSoma = Spain.pontosTuristicos.primeiroPontoTuristico.valor;
      } else if (
        dados[i].value == Spain.pontosTuristicos.segundoPontoTuristico.nome
      ) {
        valorSoma =
          valorSoma + Spain.pontosTuristicos.segundoPontoTuristico.valor;
      } else if (
        dados[i].value == Spain.pontosTuristicos.terceiroPontoTuristico.nome
      ) {
        valorSoma =
          valorSoma + Spain.pontosTuristicos.terceiroPontoTuristico.valor;
      }
    }
  }
  return valorSoma;
...
function valorT() {
  ...
  if (
    ...
    R$  ${(1147.0 * getNumeroViajantes()).toFixed(
      2
