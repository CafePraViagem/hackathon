...
function mudarFundoCalculadora() {
  ...
  imgURL[0].style.background =
    "url('" + Alemanha["image"] + "') no-repeat center";
  ...
  for (let i = 0; i < dados.length; i++) {
    if (dados[i].checked) {
      if (dados[i].value == Alemanha.hospedagem.hospedagem1.nomeHospedagem1) {
        ...
        dados[i].value == Alemanha.hospedagem.hospedagem2.nomeHospedagem2
      ) {
        valorSoma = Alemanha.hospedagem.hospedagem2.valor;
      } else if (
        dados[i].value == Alemanha.hospedagem.hospedagem3.nomeHospedagem3
      ) {
        valorSoma = Alemanha.hospedagem.hospedagem3.valor;
      } else if (
        dados[i].value == Alemanha.hospedagem.hospedagem4.nomeHospedagem4
      ) {
...
function somaTurismo() {
  ...
  for (let i = 0; i < dados.length; i++) {
    if (dados[i].checked) {
      if (
        dados[i].value == Alemanha.pontosTuristicos.primeiroPontoTuristico.nome
      ) {
        valorSoma = Alemanha.pontosTuristicos.primeiroPontoTuristico.valor;
      } else if (
        dados[i].value == Alemanha.pontosTuristicos.segundoPontoTuristico.nome
      ) {
        valorSoma =
          valorSoma + Alemanha.pontosTuristicos.segundoPontoTuristico.valor;
      } else if (
        dados[i].value == Alemanha.pontosTuristicos.terceiroPontoTuristico.nome
      ) {
        valorSoma =
          valorSoma + Alemanha.pontosTuristicos.terceiroPontoTuristico.valor;
      }
    }
  }
  return valorSoma;
...
function valorT() {
  ...
  if (
    ...
    R$  ${(1200.0 * getNumeroViajantes()).toFixed(
      2
